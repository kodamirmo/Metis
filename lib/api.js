...
// mostly for testing.
if (process.env.Soyuz_ROOT_DIR) globalConfig.root = process.env.Soyuz_ROOT_DIR;
    ...
    if(regex.test(repoURL)){
        ...
        if (!shell.which('git')) {
            ...
            async.series([
                    ...
                    function(done){
                        var appPath = data.appDirectory;
                        shell.mv(appPath + 'bundle', appPath + getFolderName(repoURL));
                        done(null,'OK');
                    },
                    function(done){
                        var appPath = data.appDirectory + getFolderName(repoURL);
                        shell.cd(appPath + '/programs/server');
